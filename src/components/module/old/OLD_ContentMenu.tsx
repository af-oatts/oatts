...
import { OLD_ContentCompletionIndicator } from "@/components/module/old/OLD_ContentCompletionIndicator";
    ...
    <>
      <ListItemButton onClick={handleClick}>
        <Box width="100%" display="grid" gridTemplateColumns="30px 1fr auto" alignItems="center">
          ...
          <OLD_ContentCompletionIndicator completion={completionStatus} />
 
        </Box>
      </ListItemButton>
      <Collapse in={open} timeout="auto" unmountOnExit>
        <List component="div" disablePadding sx={{ paddingRight: 0 }}>
          {contentItem.children?.map((c) => <OLD_ContentMenuItem key={c.id} getState={getState} contentItem={c} {...rest} />)}
        </List>
      </Collapse>
    </>
  );
...
export function OLD_ContentMenuItem(props: ContentMenuItemProps) {
    ...
    <>
      <ListItemButton {...rest} onClick={(_) => setContent(contentItem.id)} selected={isSelected(contentItem.id)}>
        <Box width="100%" display="grid" gridTemplateColumns="1fr auto" alignItems="center">
          <ListItemText primary={contentItem.name} />
          <OLD_ContentCompletionIndicator completion={getState(contentItem.id)?.completionStatus} />
        </Box>
      </ListItemButton>
    </>
  );
